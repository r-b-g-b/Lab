...
def ridge_corr(Rstim, Pstim, Rresp, Presp, alphas, normalpha=False, dtype=np.single, corrmin=0.2,
			   singcutoff=1e-10, use_corr=True):
	...
	normalpha : boolean
		Whether ridge parameters should be normalized by the Frobenius norm of Rstim. Good forcomparing models with different numbers of parameters.
	dtype : np.dtype
	...
	for na, a in zip(nalphas, alphas):
		...
		pred = np.dot(mult_diag(D, PVh, left=False), UR) ## Best (1.75s to prediction in test)
		# pred = np.dot(mult_diag(D, np.dot(Pstim, Vh.T), left=False), UR) ## Better (2.0 seconds to prediction in test)
		
		# pvhd = reduce(np.dot, [Pstim, Vh.T, D]) ## Pretty good (2.4s to prediction in test)
		# pred = np.dot(pvhd, UR)
		
		# wt = reduce(np.dot, [Vh.T, D, UR]).astype(dtype) ## Bad (14.2s to prediction in test)
		# wt = reduce(np.dot, [Vh.T, D, U.T, Rresp]).astype(dtype) ## Worst
		# pred = np.dot(Pstim, wt) ## Predict test responses

		if use_corr:
			prednorms = np.apply_along_axis(np.linalg.norm, 0, pred) ## Compute test response norms
		...
		print "Training: alpha=%0.3f, mean corr=%0.5f, max corr=%0.5f, over-under(%0.2f)=%d" % (a, np.mean(Rcorr), np.max(Rcorr), corrmin, (Rcorr>corrmin).sum()-(-Rcorr>corrmin).sum())	
	return Rcorrs

def eigridge(train_stim, valid_stim, train_resp, valid_resp, alphas, test_resp=None, test_stim = None, saveallwts = True, stop_early = False, dtype = np.single, corrmin = 0.2, covmat = None, force_cmode = None, verbose = False):
	
	"""
	Ridge regresses [train_stim] onto [train_resp] for each ridge parameter in [alpha].  Returns the fit linear weights for each alpha, as well as the distributions of correlations on a held-out validation set ([valid_stim] and [valid_resp]).  Note that these should NOT be the "real" held-out test set, only a small validation set used to find the optimal ridge parameter.
	
	If an [test_resp] and [test_stim] are given, correlations on that dataset will be computed and displayed for each alpha.
	
	If [savallewts] is True, all weights will be returned.  Otherwise only the best weights will be returned.
	
	If [stop_early] is True, the weights and correlations will be returned as soon as the mean correlation begins to drop.  Does NOT imply early-stopping in the regularized regression sense.
	
	The given [dtype] will be applied to the regression weights as they are computed.

	If given a [covmat], it will be used instead of the stimulus covariance matrix.
	"""

	if force_cmode is not None:
		cmode = force_cmode
	else:
		cmode = train_stim.shape[0]<train_stim.shape[1]

	if verbose: print "Cmode =",cmode
	
	# Prespnorms = np.apply_along_axis(np.linalg.norm, 0, valid_resp) ## Precompute test response norms

	if verbose: print "Doing Eigenvalue decomposition..."
	if cmode:
		if covmat is None:
			covmat = np.array(np.dot(train_stim.astype(dtype), train_stim.T.astype(dtype)).todense())
		
		[S,U] = np.linalg.eigh(covmat)
		U1 = np.dot(train_stim.T, U)
		U2 = np.dot(U.T, train_resp)
	else:
		if covmat is None:
			covmat = np.array(np.dot(train_stim.T.astype(dtype), train_stim.astype(dtype)).todense())
		
		[S,U] = np.linalg.eigh(covmat)
		# Usr = reduce(np.dot, [U.T, train_stim.T, train_resp])
		Usr = np.dot(U.T, np.dot(train_stim.T.tocsr(), sparse.csc_matrix(train_resp)).todense())



	if verbose: print "Eigenvector shape =",U.shape
	...
	bestcorr = -1.0
	for a in alphas:
		if verbose: print "Running alpha %0.3f"%a
		D = np.diag(1/(S+a)).astype(dtype)
		
		if cmode:
			wt = np.array(reduce(np.dot, [U1, D, U2]).astype(dtype))
		else:
			wt = np.array(reduce(np.dot, [U, D, Usr]).astype(dtype))
		
		pred = np.dot(valid_stim.tocsr(), sparse.csc_matrix(wt)) ## Predict test responses

		Rcorr = np.array([np.corrcoef(valid_resp[:,ii], np.array(pred[:,ii].todense()).ravel())[0,1] for ii in range(valid_resp.shape[1])]) ## Slowly compute correlations
		...
		if verbose: print "Training: alpha=%0.4e, mean corr=%0.3f, max corr=%0.3f, over-under(%0.2f)=%d" % (a, np.mean(Rcorr), np.max(Rcorr), corrmin, (Rcorr>corrmin).sum()-(-Rcorr>corrmin).sum())
		
		## Test alpha on real test set if given
		if test_resp is not None and test_stim is not None:
			rpred = np.dot(test_stim, wt)
			Pcorr = np.array([np.corrcoef(test_resp[:,ii], rpred[:,ii].ravel())[0,1] for ii in range(test_resp.shape[1])])
			Pcorr[np.isnan(Pcorr)] = 0.0
			if verbose: print "Testing: alpha=%0.3f, mean corr=%0.3f, max corr=%0.3f" % (a, np.mean(Pcorr), np.max(Pcorr))
			Pcorrs.append(Pcorr)
...
def bootstrap_ridge(Rstim, Rresp, Pstim, Presp, alphas, nboots, chunklen, nchunks, dtype=np.single, corrmin=0.2, joined=None, singcutoff=1e-10, normalpha=False, single_alpha=False, use_corr=True):
	"""Uses ridge regression with a bootstrapped held-out set to get optimal alpha values for each response.
	[nchunks] random chunks of length [chunklen] will be taken from [Rstim] and [Rresp] for each regression run.  [nboots] total regression runs will be performed.  The best alpha value for each response will be averaged across the bootstraps to estimate the best alpha for that 
	response.
	...
	chunklen : int
		On each sample, the training data is broken into chunks of this length. This should be a 
		few times longer than your delay/STRF. e.g. for a STRF with 3 delays, I use chunks of 
		length 10.
	nchunks : int
		The number of training chunks held out to test ridge parameters for each bootstrap sample. 
		The product of nchunks and chunklen is the total number of training samples held out for 
		each sample, and this  product should be about 20 percent of the total length of the 
		training data.
	dtype : np.dtype
		All data will be cast as this dtype for computation. np.single is used by default for memory
		efficiency, as using np.double will thrash most machines on a big problem. If you want to do
		regression on  complex variables, this should be changed to np.complex128.
	corrmin : float in [0..1]
		Purely for display purposes. After each alpha is tested for each bootstrap sample, the 
		number of responses with correlation greater than this value will be printed. For 
		long-running regressions this can give a rough sense of how well the model works before it's 
		done.
	joined : None or list of array_like indices
		If you want the STRFs for two (or more) responses to be directly comparable, you need to 
		ensure that the regularization parameter that they use is the same. To do that, supply a 
		list of the response sets that should use the same ridge parameter here. For example, if you
		have four responses, joined could be [np.array([0,1]), np.array([2,3])], in which case 
		responses 0 and 1 will use the same ridge parameter (which will be parameter that is best on
		average for those two), and likewise for responses 2 and 3.
	singcutoff : float
