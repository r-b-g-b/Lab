...
#------------------------------------------------------------------
def computeVoronoiDiagram(inpoints):
	""" Takes a list of point objects (which must have x and y fields).
        Returns a 3-tuple of:
	...
	for inpoint in inpoints:
		points.append(Point(inpoint))
	siteList = SiteList(points)
	context  = Context()
	voronoi(siteList,context)
	context.vertices = np.array(context.vertices)
	context.lines = np.array(context.lines)
	context.edges = np.array(context.edges)
